...
const getGroupAdmins = (participants) => {
	...
	if (!isAdmin && !text) return m.reply(`Explain what's the matter?\nand Example : *${use Prefix + command} report spam*`)
	if (!isAdmin && (text.length <= 11 || args.length < 4)) throw `It's short for oi, if it's not important, don't worry `
