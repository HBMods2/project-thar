...
let handler = async (m, { conn, text, args, usedPrefix, command }) => {
	...
	try {
		...
		try {
			...
			if (anu.result.link.size.slice(-2) == "GB") return m.reply(`Don't worry.\nWhere can I post a video? ${anu.result.link.size}`)
			...
			try {
				...
				if (!res.ok) throw new e()
				let anu = await res.json()
				if (anu.result.size.slice(-2) == "GB") return m.reply(`Don't worry.\nWhere can I send a video ${anu.result.size}`)
				...
				try {
					...
					if (anu.size.slice(-2) == "GB") return m.reply(`Don't worry.\nWhere can I send a video ${anu.size}`)
