...
`.trim()
    conn.tebakbendera[id] = [
        ...
        json, poin,
        setTimeout(() => {
            if (conn.tebakbendera[id]) conn.sendButton(m.chat, `Time is up!\nThe answer is *${json.name}*`, packname + ' - ' + author, ['tebakbendera', `${usedPrefix}tebakbendera`], conn.tebakbendera[id][0])
