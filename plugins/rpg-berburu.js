...
let handler = async (m, { conn, usedPrefix }) => {
	...
	if (new Date - user.lasthunt <= cooldown) return m.reply(`📍 Enough hunting for now\nTake the time to rest, the next hunt can start in . . .\n🕖 *${((user.lasthunt + cooldown) - new Date()).toTimeString()}*`)
	if (user.armor == 0 || user.sword == 0 || user.bow == 0) return m.reply(`You need *${usedPrefix}craft* armor, sword, and bow first.\n\nYou have :\n━ ${user.armor} Armor\n━ ⚔️ ${user.sword} Sword\n━ 🏹 ${user.bow} Bow`)
	let buruan = [
 ...
 *🐊 = [ ${buruan[5].hewan} ]*			 *🐓 = [ ${buruan[11].hewan} ]*`
	...
	setTimeout(() => {
		...
		conn.sendButton(m.chat, hsl, packname + ' - ' + author, gmbrt, [[`Cage`, `${usedPrefix}cage`]], m)
	}, cooldownn)
					 
	setTimeout(() => {
		m.reply('_The Hunt Begins..._')
	}, 0)
